...
/**
 * The video side drops the degree, the internship and the certificates.
 * Nobody commissioning an edit cares about a CGPA — showing it there only
 * makes the page longer without making the case stronger.
 */
export default function About({ mode }) {
  const isDev = mode !== "video";
  const skillGroups = isDev ? skills.dev : skills.video;
  ...
  return (
    ...
    >
      <div className={`grid gap-12 ${isDev ? "lg:grid-cols-12 lg:gap-16" : ""}`}>
        {/* Bio */}
        <div className={isDev ? "lg:col-span-5" : "max-w-3xl"}>
        ...
        {/* Timeline — development side only */}
        {isDev && (
